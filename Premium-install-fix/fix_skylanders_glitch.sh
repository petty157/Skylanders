...
patch_file() {
    ...
    sed 's/\('"$TARGET_ATTR"'[[:space:]]*=[[:space:]]*"\)[^"]*\(".*\)/\1'"$NEW_VALUE"'\2/I' \
        "$FILE" > "$TMP_FILE" 2>/dev/null

    if ! grep -q "$NEW_VALUE" "$TMP_FILE" 2>/dev/null; then
        sed 's/\('"$TARGET_ATTR"'[[:space:]]*=[[:space:]]*"\)[^"]*\(".*\)/\1'"$NEW_VALUE"'\2/' \
            "$FILE" > "$TMP_FILE"
    fi

    if ! grep -q "$NEW_VALUE" "$TMP_FILE" 2>/dev/null; then
        echo "  Patch failed — file is unchanged."
        rm -f "$TMP_FILE"
        return 1
    fi
